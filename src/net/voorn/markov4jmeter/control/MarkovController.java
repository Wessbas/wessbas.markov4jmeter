...
/**
 * <p>
 * The class represents a Markov Model and contains the definition of the
 * behavior mix to use during execution as well as the specification of the
 * number of allowed parallel active instances of the Markov Model (each
 * representing a user session) during test run. It must only contain
 * ApplicationState elements as direct child elements.
 * </p>
 * 
 * <p>
 * An instance of the controller executes as follows:
 * <ul>
 * <li>When initialized by the JMeter engine, a behavior model is requested from
 * the <i>session arrival controller</i>. The entry state is set according to
 * the user behavior's entry state.</li>
 * <li>When the first iteration is requested by the engine (i.e. the engine
 * calls the method <i>next()</i>), the controller waits inside the <i>session
 * arrival controller</i>'s queue to be granted session entrance. As soon as the
 ...
 * the next state by first requesting the set of outgoing transitions from the
 * current state, determining the set of transitions whose guards evaluate to
 * true, and randomly choosing a transtion from the set based on the
 ...
 */
public class MarkovController extends
		org.apache.jmeter.control.GenericController implements ThreadListener,
		TestStateListener, TestIterationListener {
	...
	/*
	 * Contains user behavior mixes for all applications (during test
	 * execution). This map is cleared after each test execution.
	 */
	private static Map<Integer, BehaviorMix> applicationBehaviorMixes = Collections
			.synchronizedMap(new HashMap());

	// the session trace that is simulated
	private SessionData sessionData;

	// index of the useCase in sessionTrace
	private int sessionIndex;

	/**
	 * To make sure that exitSession of the session arrival controller is called
	 * only once.
	 */
	 ...
	 * 
	 * @param enabled
	 *            true if a session arrival controller shall be used.
	 */
	 ...
	 * listener when changes in terms of application state changes occur. We use
	 * this list for generating the behavior file templates in GUI mode.
	 * 
	 * @param names
	 *            the state names.
	 */
	...
	private void setEntryState() {
		...
		// set the entry state based on the first entry in sessionData
		for (int i : this.stateMap.keySet()) {
			ApplicationState appState = this.stateMap.get(i);
			if (appState.getName().contains(
					this.sessionData.getUseCases().get(0).getName())) {
				System.out.println("Set Entry state: " + appState.getName());
	...
	private Map<String, Integer> getStateName2IdMap() {
		Map<String, Integer> map = new HashMap(
				subControllersAndSamplers.size(), 1);
		for (int i = 0; i < subControllersAndSamplers.size(); i++) {
			TestElement elem = subControllersAndSamplers.get(i);
	...
	private void initializeBehaviorMix() throws BehaviorException {
		...
		synchronized (this.getClass()) {
			...
			this.behaviorMix = MarkovController.applicationBehaviorMixes
					.get(new Integer(this.getId()));
			if (this.behaviorMix == null) {
				...
				/* Add to map */
				MarkovController.applicationBehaviorMixes.put(
						new Integer(this.getId()), this.behaviorMix);
			}
		}
	}
	 ...
	 * 
	 * @throws JMeterStopTestException
	 *             when an error occurs.
	 * @throws JMeterStopThreadException
	 *             when an error occurs.
	 */
	private void assignBehavior() throws JMeterStopTestException,
		...
		} catch (final BehaviorException ex) {
	...
	public void initialize() throws JMeterStopTestException {
		...
		// TODO: there is still an open issue with a test being ed when
		// multiple virtual users are active; if several threads are running,
		// e.g., 20 virtual users, the JMeterStopTestException which is possibly
		// thrown at the end of this method, will not cause all threads being
		// stopped immediately; in case a behavior model is invalid, the same
		// related error message will be logged multiple times in the GUI and
		// the JMeter log-file as well; this was already an issue in
		// Markov4JMeter 1.0 which wrote multiple entries into the log only;
		...
		if (this.sessionArrivalControllerEnabled) {
			this.sessionArrivalController = SessionArrivalController
					.getInstance();
			/* dangerous since several thread access this object but it works */
			this.sessionArrivalController.init(
					this.isArrivalCtrlLoggingEnabled(),
					this.getArrivalCtrlLogfile());
		}

		// get sessionData
		SessionFile sessionFile = SessionFile.getInstance();
		this.sessionData = sessionFile.getSessionInstance();
		this.sessionIndex = 0;
		...
		} catch (final JMeterStopTestException ex) {
	...
	/**
	 * Reinitialize instance for example after a test iteration. A behavior is
	 ...
	 */
	@Override
	protected void reInitialize() throws JMeterStopTestException,
			JMeterStopThreadException {
		super.reInitialize();
		this.assignBehavior();
		this.setEntryState();
		...
		while (actionsTokenizer.hasMoreTokens()) {
			...
			try {
				if (actionTokenizer.countTokens() == 2) {
					...
					rightSide = evaluateExpression(actionTokenizer.nextToken()
							.trim());
	...
	public void transition() {
		...
		Map<Integer, Double> transitionsProbabilitiesMap;
		Map<Integer, ThinkTime> transitionsThinkTimesMap;
		...
		transitionsProbabilitiesMap = this.behavior
				.getStateTransitionProbabilitiesMap(this.currentState.getId());
		transitionsThinkTimesMap = this.behavior
				.getStateTransitionThinkTimesMap(this.currentState.getId());
		...
		List<ApplicationStateTransition> transitionslist = transitions
				.getTransitionsAsList();
		Iterator<ApplicationStateTransition> it = transitionslist
				.listIterator();
		/*
		 * add final state 0 to truelist and init curCumProb with its
		 * probability
		 */
		truelist.add(null);
		double curCumProb = transitionsProbabilitiesMap.get(0);
		cumProbList.add(curCumProb);

		// long startTime = System.currentTimeMillis();

		ConditionEvaluationStatistics ces = ConditionEvaluationStatistics
				.getInstance();

		// for (int i = 0; it.hasNext(); i++) {
		// ApplicationStateTransition cur = it.next();
		//
		// double curProb = transitionsProbabilitiesMap.get(cur
		// .getDstStateId());
		// if (!cur.isDisabled() && !(curProb <= 0)
		// && evaluateCondition(cur.getGuard())) {
		// truelist.add(cur);
		// curCumProb += curProb;
		// cumProbList.add(curCumProb);
		// ces.addConditionEntry(this.currentState.getName(),
		// cur.getDstStateName(), 1);
		// } else if (!cur.isDisabled() && !(curProb <= 0)) {
		// ces.addConditionEntry(this.currentState.getName(),
		// cur.getDstStateName(), 0);
		// }
		// }

		// for each possible transition check if it is true
		for (int i = 0; it.hasNext(); i++) {
			ApplicationStateTransition cur = it.next();
			...
			if (!cur.isDisabled() && !(curProb <= 0)) {
				...
				curCumProb += curProb;
				cumProbList.add(curCumProb);
				if (evaluateCondition(cur.getGuard())) {
					ces.addConditionEntry(this.currentState.getName(),
							cur.getDstStateName(), 1);
				} else {
					ces.addConditionEntry(this.currentState.getName(),
		...
		// long getNextTransitionDuration = System.currentTimeMillis() -
		// startTime;
		...
		ApplicationStateTransition nextTransition = null;
		;
		...
		// search next state
		if (sessionIndex < sessionData.getUseCases().size() - 1) {
			this.sessionIndex++;
			String nextState = sessionData.getUseCases().get(sessionIndex)
					.getName();

			it = transitionslist.listIterator();
			for (int i = 0; it.hasNext(); i++) {
				ApplicationStateTransition cur = it.next();
				if (cur.getDstStateName().contains(nextState)) {
					nextTransition = cur;
					System.out.println("Set next state:"
							+ nextTransition.getDstStateName());
					break;
				}
			}
		}
		...
		final ApplicationState oldState = this.currentState;

		this.currentState = nextTransition != null ? this.stateMap
				.get(nextTransition.getDstStateId()) : null;

		// after the new current state has been determined, the think time
		// which might be alternatively modeled via "Gaussian Random Timer"
		// will cause a delay at this point -> insert own think time delay here;

		// if (this.currentState != null) {
		//
		// final ThinkTime thinkTime = transitionsThinkTimesMap
		// .get(this.currentState.getId());
		//
		// // old matrix format did not support think times, in that case null
		// // will be returned;
		// if (thinkTime != null) {
		//
		// // final long delay = thinkTime.getDelay();
		// // final long newDelay = ((delay - getNextTransitionDuration) <
		// // 0) ? 0L : (delay - getNextTransitionDuration);
		//
		// final long delay = thinkTime.getDelay();
		//
		// // final long delay = ((thinkTime.getDelay() -
		// // getNextTransitionDuration) < 0) ? 0L : (thinkTime.getDelay()
		// // - getNextTransitionDuration);
		//
		// try {
		//
		// // throws an IllegalArgument- or InterruptedException;
		// Thread.sleep(delay);
		//
		...
		// System.out.println(this.currentState!=null?this.currentState.getName():new
		// String("<EXIT>"));
	}
	...
	public Sampler next() throws JMeterStopTestException {
		...
		if (this.sessionArrivalControllerEnabled && !this.inSession) {
			...
			try {
				/**  */
				this.sessionArrivalController.enterSession(Integer
						.parseInt(this.getArrivalCtrlNumSessions()));
				this.inSession = true;
			} catch (Exception ex) {
		...
		/* exit session. */
		/**
		 * @TODO: check why returnValue==null occurs twice (reason for variable
		 *        mustReduceNumThreads)
		 */
		...
		if (returnValue instanceof HTTPSamplerProxy) {
			HTTPSamplerProxy hTTPSamplerProxy = (HTTPSamplerProxy) returnValue;
			if (hTTPSamplerProxy.getName().contains("INITIAL")) {
				...
				for (int i = 0; i < arguments.getArgumentCount(); i++) {
					...
					if (argument instanceof HTTPArgument) {
						...
						hTTPArgument.setValue(hTTPArgument.getValue() + "_"
								+ this.behavior.getBName());
					}
				}
			}
		}
